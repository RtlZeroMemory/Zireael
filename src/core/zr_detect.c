...
static void zr_detect_apply_parsed(zr_terminal_profile_t* profile, const zr_detect_parsed_t* parsed) {
  ...
  if (parsed->da1_responded != 0u) {
    ...
    profile->supports_sixel = (parsed->da1_has_sixel != 0u);
  }
...
static void zr_detect_apply_flags(zr_terminal_profile_t* profile, plat_caps_t* caps, zr_terminal_cap_flags_t flags) {
  ...
  profile->supports_sixel = ((flags & ZR_TERM_CAP_SIXEL) != 0u);
  profile->supports_kitty_graphics = ((flags & ZR_TERM_CAP_KITTY_GRAPHICS) != 0u);
  profile->supports_iterm2_images = ((flags & ZR_TERM_CAP_ITERM2_IMAGES) != 0u);
  profile->supports_underline_styles = ((flags & ZR_TERM_CAP_UNDERLINE_STYLES) != 0u);
  profile->supports_colored_underlines = ((flags & ZR_TERM_CAP_COLORED_UNDERLINES) != 0u);
  profile->supports_hyperlinks = ((flags & ZR_TERM_CAP_HYPERLINKS) != 0u);
  profile->supports_grapheme_clusters = ((flags & ZR_TERM_CAP_GRAPHEME_CLUSTERS) != 0u);
  profile->supports_overline = ((flags & ZR_TERM_CAP_OVERLINE) != 0u);
  profile->supports_pixel_mouse = ((flags & ZR_TERM_CAP_PIXEL_MOUSE) != 0u);
  profile->supports_kitty_keyboard = ((flags & ZR_TERM_CAP_KITTY_KEYBOARD) != 0u);

  caps->supports_mouse = ((flags & ZR_TERM_CAP_MOUSE) != 0u);
  caps->supports_bracketed_paste = ((flags & ZR_TERM_CAP_BRACKETED_PASTE) != 0u);
  caps->supports_focus_events = ((flags & ZR_TERM_CAP_FOCUS_EVENTS) != 0u);
  caps->supports_osc52 = ((flags & ZR_TERM_CAP_OSC52) != 0u);
  caps->supports_sync_update = ((flags & ZR_TERM_CAP_SYNC_UPDATE) != 0u);
  caps->supports_scroll_region = ((flags & ZR_TERM_CAP_SCROLL_REGION) != 0u);
  caps->supports_cursor_shape = ((flags & ZR_TERM_CAP_CURSOR_SHAPE) != 0u);
  caps->supports_output_wait_writable = ((flags & ZR_TERM_CAP_OUTPUT_WAIT_WRITABLE) != 0u);
