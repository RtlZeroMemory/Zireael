...
static zr_result_t zr_engine_drain_platform_input(zr_engine_t* e, uint32_t time_ms, bool* out_had_input) {
  if (!e || !e->plat || !out_had_input) {
    return ZR_ERR_INVALID_ARGUMENT;
  }
...
static zr_result_t zr_engine_present_render(zr_engine_t* e,
  ...
  const uint64_t t0_us = zr_engine_now_us_estimate();
  zr_result_t rc =
      zr_diff_render(&e->fb_prev, diff_next, &e->caps, &e->term_state, e->cfg_runtime.enable_scroll_optimizations,
                     diff_out, diff_cap, &diff_len, final_ts, stats);
...
static int zr_engine_poll_wait_and_fill(zr_engine_t* e, int timeout_ms, uint32_t time_ms) {
  ...
  rc = zr_engine_drain_platform_input(e, time_ms, &had_input);
  if (rc != ZR_OK) {
    return (int)rc;
  }
  if (!had_input) {
