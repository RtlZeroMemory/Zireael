...
zr_result_t engine_create(zr_engine_t** out_engine, const zr_engine_config_t* cfg) {
  ...
  *out_engine = NULL;
  ...
  e->out_cap = (size_t)cfg->limits.out_max_bytes_per_frame;
  ...
  rc = zr_arena_init(&e->arena_persistent, (size_t)cfg->limits.arena_initial_bytes,
                     (size_t)cfg->limits.arena_max_total_bytes);
  if (rc != ZR_OK) {
    goto cleanup;
  }
  rc = zr_arena_init(&e->arena_frame, (size_t)cfg->limits.arena_initial_bytes,
                     (size_t)cfg->limits.arena_max_total_bytes);
  if (rc != ZR_OK) {
    goto cleanup;
  }
  ...
  rc = zr_event_queue_init(&e->evq, e->ev_storage, e->ev_cap, e->user_bytes, e->user_bytes_cap);
  if (rc != ZR_OK) {
    goto cleanup;
  }

  rc = plat_create(&e->plat, &cfg->plat);
  if (rc != ZR_OK) {
    goto cleanup;
  }
  ...
  rc = plat_get_size(e->plat, &e->size);
  if (rc != ZR_OK) {
    goto cleanup;
  }
...
zr_result_t engine_present(zr_engine_t* e) {
  ...
  rc = plat_write_output(e->plat, e->out_buf, (int32_t)out_len);
  if (rc != ZR_OK) {
    return rc;
  }
...
zr_result_t engine_set_config(zr_engine_t* e, const zr_engine_runtime_config_t* cfg) {
  ...
  const bool want_out_buf = (cfg->limits.out_max_bytes_per_frame != e->cfg_runtime.limits.out_max_bytes_per_frame);
  const bool want_arena_reinit =
      (cfg->limits.arena_initial_bytes != e->cfg_runtime.limits.arena_initial_bytes) ||
  ...
  if (want_arena_reinit) {
    rc = zr_arena_init(&arena_frame_new, (size_t)cfg->limits.arena_initial_bytes,
                       (size_t)cfg->limits.arena_max_total_bytes);
    if (rc != ZR_OK) {
      goto cleanup;
    }
