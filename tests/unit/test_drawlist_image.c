/*
  tests/unit/test_drawlist_image.c — Unit tests for drawlist v5 DRAW_IMAGE.

  Why: DRAW_IMAGE is version-gated and has protocol/fallback branches; these
...
ZR_TEST_UNIT(drawlist_image_validate_v5_accepts_basic_rgba) {
  ...
  const size_t len = zr_make_draw_image_drawlist(bytes, ZR_DRAWLIST_VERSION_V5, &cmd, blob, 4u);
  ZR_ASSERT_EQ_U32(zr_validate_draw_image(bytes, len, &view), ZR_OK);
}

ZR_TEST_UNIT(drawlist_image_validate_v4_rejects_opcode) {
  uint8_t blob[4] = {9u, 8u, 7u, 255u};
  uint8_t bytes[160];
  zr_dl_cmd_draw_image_t cmd;
  zr_dl_view_t view;
