...
ZR_TEST_UNIT(engine_poll_events_flushes_pending_escape_on_wake_without_input) {
  mock_plat_reset();
  mock_plat_set_size(80u, 24u);
  ...
  const uint8_t esc_only[] = {0x1Bu};
  ZR_ASSERT_EQ_U32(mock_plat_push_input(esc_only, sizeof(esc_only)), ZR_OK);
  ZR_ASSERT_EQ_U32((uint32_t)engine_poll_events(e, 0, out, (int)sizeof(out)), 0u);

  mock_plat_force_wake();
  const int n = engine_poll_events(e, 1, out, (int)sizeof(out));
  ZR_ASSERT_TRUE(n > 0);
  ...
  const uint32_t key = zr_load_u32le(out + sizeof(zr_evbatch_header_t) + sizeof(zr_ev_record_header_t) + 0u);
  ZR_ASSERT_EQ_U32(key, (uint32_t)ZR_KEY_ESCAPE);
