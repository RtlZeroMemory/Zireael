...
static void zr_fill_fb_ascii(zr_test_ctx_t* ctx, zr_fb_t* fb, uint8_t ch) {
  ...
  const zr_style_t s = (zr_style_t){1u, 2u, 3u};
...
static void zr_assert_fb_continuations_valid(zr_test_ctx_t* ctx, const zr_fb_t* fb) {
  ...
  for (uint32_t y = 0u; y < fb->rows; y++) {
    for (uint32_t x = 0u; x < fb->cols; x++) {
      ...
      ZR_ASSERT_TRUE(c != NULL);
      if ((c->flags & ZR_FB_CELL_FLAG_CONTINUATION) != 0u) {
        ZR_ASSERT_TRUE(x > 0u);
        ZR_ASSERT_EQ_U32(c->glyph_len, 0u);
        const zr_fb_cell_t* lead = zr_fb_cell_at_const(fb, x - 1u, y);
        ZR_ASSERT_TRUE(lead != NULL);
        ZR_ASSERT_TRUE((lead->flags & ZR_FB_CELL_FLAG_CONTINUATION) == 0u);
      }
    }
  }
}
...
static void zr_assert_overlay_cell(zr_test_ctx_t* ctx, const zr_fb_t* fb, uint32_t x, uint32_t y,
  ...
  ZR_ASSERT_TRUE((c->flags & ZR_FB_CELL_FLAG_CONTINUATION) == 0u);
}

ZR_TEST_UNIT(overlay_renders_expected_ascii_within_4x40_region) {
  zr_fb_cell_t cells[50u * 6u];
  zr_fb_t fb;
  ZR_ASSERT_EQ_U32(zr_fb_init(&fb, cells, 50u, 6u), ZR_OK);
...
ZR_TEST_UNIT(overlay_clips_to_small_framebuffer) {
  zr_fb_cell_t cells[3u * 2u];
  zr_fb_t fb;
  ZR_ASSERT_EQ_U32(zr_fb_init(&fb, cells, 3u, 2u), ZR_OK);
  zr_fill_fb_ascii(ctx, &fb, (uint8_t)'.');
...
ZR_TEST_UNIT(overlay_does_not_split_wide_glyph_across_right_edge) {
  zr_fb_cell_t cells[41u * 1u];
  zr_fb_t fb;
  ZR_ASSERT_EQ_U32(zr_fb_init(&fb, cells, 41u, 1u), ZR_OK);
  zr_fill_fb_ascii(ctx, &fb, (uint8_t)'.');

  const zr_style_t s = (zr_style_t){9u, 9u, 9u};
  const uint8_t wide[] = {0xE7u, 0x95u, 0x8Cu}; /* U+754C '界' (typically width=2) */
  ZR_ASSERT_EQ_U32(zr_fb_draw_text_bytes(&fb, 39, 0, wide, sizeof(wide), &s, zr_fb_full_clip(&fb)),
                   ZR_OK);

  const zr_fb_cell_t* lead_before = zr_fb_cell_at_const(&fb, 39u, 0u);
  const zr_fb_cell_t* cont_before = zr_fb_cell_at_const(&fb, 40u, 0u);
  ZR_ASSERT_TRUE(lead_before != NULL && cont_before != NULL);
  ZR_ASSERT_TRUE((cont_before->flags & ZR_FB_CELL_FLAG_CONTINUATION) != 0u);
  ...
  const zr_fb_cell_t* lead_after = zr_fb_cell_at_const(&fb, 39u, 0u);
  const zr_fb_cell_t* cont_after = zr_fb_cell_at_const(&fb, 40u, 0u);
  ZR_ASSERT_TRUE(lead_after != NULL && cont_after != NULL);

  /* Cell 40 is outside overlay_cols; must not change. */
  ZR_ASSERT_TRUE((cont_after->flags & ZR_FB_CELL_FLAG_CONTINUATION) != 0u);
