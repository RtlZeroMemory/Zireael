...
#define ZR_TEST_NAMED(ident, name_str)                                           \
  ...
  __declspec(allocate(".CRT$XCU")) static const zr_test_ctor_fn_t                \
      zr_test_ctor__##ident = zr_test_reg__##ident;                              \
  static void __cdecl zr_test_reg__##ident(void) { zr_test_register((name_str), ident); } \
  static void ident(zr_test_ctx_t* ctx)
#elif defined(__GNUC__) || defined(__clang__)
#define ZR_TEST_NAMED(ident, name_str)                                           \
  static void ident(zr_test_ctx_t* ctx);                                         \
  static void zr_test_reg__##ident(void) __attribute__((constructor));           \
  static void zr_test_reg__##ident(void) { zr_test_register((name_str), ident); } \
  static void ident(zr_test_ctx_t* ctx)
#else
...
#define ZR_ASSERT_EQ_U32(a, b)                                                  \
  do {                                                                          \
    ...
    if (_zr_a != _zr_b) {                                                       \
      zr_test_failf((ctx), __FILE__, __LINE__,                                  \
                    "ZR_ASSERT_EQ_U32 failed: %s=%u %s=%u", #a,                  \
                    (unsigned)_zr_a, #b, (unsigned)_zr_b);                       \
      return;                                                                   \
    }                                                                           \
  } while (0)
...
#define ZR_ASSERT_MEMEQ(actual_ptr, expected_ptr, len)                           \
  do {                                                                           \
    ...
    if (!zr_test_memeq(_zr_a_ptr, _zr_e_ptr, _zr_len)) {                          \
      zr_test_failf((ctx), __FILE__, __LINE__,                                   \
                    "ZR_ASSERT_MEMEQ failed: %s vs %s (len=%zu)",                \
                    #actual_ptr, #expected_ptr, _zr_len);                        \
      return;                                                                    \
    }                                                                            \
  } while (0)
